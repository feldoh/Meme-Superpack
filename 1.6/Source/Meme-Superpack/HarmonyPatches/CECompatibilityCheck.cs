...
namespace MSS.MemeSuperpack.HarmonyPatches
{
	...
	public static class CECompatibilityCheck
	{
		...
		{
			if (!MemeSuperpackMod.settings.ceSpam)
				return;
			Log.Message("Checking CE Compatibility.");
			var modCECompatibilityList =
				DefDatabase<ModCECompatibilityList>
					.GetNamed("MSSMeme_ModCECompatibilityList", false)
					?.compatibleModPackageIds?.ToHashSet() ?? new HashSet<string>();
			modCECompatibilityList.AddRange(
				DefDatabase<ModCECompatibility>
					.AllDefs.Select(mcp => mcp.modContentPack.PackageId)
					.ToHashSet()
			);
			...
			foreach (ModMetaData mod in ModLister.AllInstalledMods.Where(mmd => mmd.Active))
			{
				...
				(
					modCECompatibilityList.Contains(mod.PackageId) ? ceCompatibleMods : ceIncompatibleMods
				).Add(modDesc);
				if (ceIncompatibleMods.Contains(modDesc))
					Log.Message($"{modDesc}) is not CE Compatible... for shame!");
			}

			Log.Message(
				$"{ceCompatibleMods.Count} / {ceCompatibleMods.Count + ceIncompatibleMods.Count} are CE Compatible."
			);
