...
{
	public new CompProperties_ExplosiveWithTerrain Props =>
		(CompProperties_ExplosiveWithTerrain)props;

	public override void PostDestroy(DestroyMode mode, Map previousMap)
	{
		var radius = ExplosiveRadius();
		if (radius <= 0f || mode != DestroyMode.KillFinalize || !Props.explodeOnKilled)
			return;
