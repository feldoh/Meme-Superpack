...
public class CompBuildingAura : ThingComp
{
	...
	public override void CompTick()
	{
		base.CompTick();
		if (
			!MemeSuperpackMod.settings.buildingAura
			|| parent.Position == _nextPossibleBuildPosition
	...
	{
		if (
			!_nextPossibleBuildPosition.IsValid
			|| !_nextPossibleBuildPosition.Standable(parent.MapHeld)
			|| _nextPossibleBuildPosition.GetFirstBuilding(parent.MapHeld) != null
			|| _nextPossibleBuildPosition.GetThingList(parent.MapHeld).Any(t => t.def.IsBlueprint)
		)
			return;
		ThingDef stuff = Props.stuff.RandomElement();
		ThingDef def = Props
			.buildables.Where(b =>
				GenConstruct.CanBuildOnTerrain(
					b,
					_nextPossibleBuildPosition,
					parent.Map,
					Rot4.Random,
