...
public class IncidentWorker_Homeless : IncidentWorker
{
	...
	protected override bool CanFireNowSub(IncidentParms parms) =>
		MemeSuperpackMod.settings.bedFires
		&& base.CanFireNowSub(parms)
		&& parms.target is Map map
		&& map.listerBuildings.allBuildingsColonist.Exists(b =>
			b.def.IsBed && b.FlammableNow && b.GetAssignedPawn() != null
		);
	...
	protected override bool TryExecuteWorker(IncidentParms parms)
	{
		...
		var beds = target
			.listerBuildings.allBuildingsColonist.Where(b =>
				b.def.IsBed && b.FlammableNow && b.GetAssignedPawn() != null
			)
			.ToList();
		if (beds.Count <= 0)
		...
		foreach (Building b in beds)
		{
			...
			FireUtility.TryStartFireIn(position, target, Rand.Range(0.420f, 0.69f), instigator: null);
		}

		if (SendLetter)
			SendStandardLetter(parms, new LookTargets(beds));
		return true;
	}
}
