...
public class HediffComp_SkillIssue : HediffComp
{
	...
	{
		foreach (
			SkillRecord skill in Pawn.skills.skills.Where(s =>
	...
	public override string CompDescriptionExtra =>
		"\n\nSkill Issues:\n"
		+ Pawn.skills.skills.Where(s => Props.allSkills || Props.skillLosses.ContainsKey(s.def))
			.Select(s =>
				$"\t{s.def.LabelCap}:\t{Props.skillLosses.GetWithFallback(s.def, Props.defaultSkillLoss)}"
			)
			.ToLineList();
}
