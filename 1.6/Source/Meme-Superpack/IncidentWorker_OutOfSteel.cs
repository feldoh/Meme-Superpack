...
public class IncidentWorker_OutOfSteel : IncidentWorker
{
	...
	protected override bool CanFireNowSub(IncidentParms parms) =>
		MemeSuperpackMod.settings.steelLoss
	...
	public void TryStartFire(Map map, IntVec3 cell)
	{
		foreach (IntVec3 radialPatternInRadius in GenRadial.RadialPatternInRadius(5f))
		{
			...
			if (
				!c.InBounds(map)
				|| !(FireUtility.ChanceToStartFireIn(c, map) > 0)
				|| !GenSight.LineOfSight(cell, c, map, true)
			)
				continue;
			if (Rand.Chance(0.1f))
				FilthMaker.TryMakeFilth(c, map, ThingDefOf.Filth_Fuel);
			if (Rand.Chance(0.4f))
				FireUtility.TryStartFireIn(c, map, Rand.Range(0.3f, 0.8f), instigator: null);
		}
	}
	...
	protected override bool TryExecuteWorker(IncidentParms parms)
	{
		...
		if (
			!StockpileUtility.FindStockpileWithItem(
				parms.target as Map,
				ThingDefOf.Steel,
				100,
				out Zone zone
		...
		if (SendLetter && destroyed > 0)
			SendStandardLetter(
				parms,
				new LookTargets(zone.Position, target),
				new NamedArgument(destroyed, "DESTROYED"),
