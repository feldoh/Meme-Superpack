...
{
	public Lazy<HashSet<ThoughtDef>> WatchedRimRimThoughts =
		new(
			() =>
				new HashSet<ThoughtDef>()
				{
	...
	[ItemCanBeNull]
	public Lazy<ResearchProjectDef> ElectricityResearch =
		new(() => DefDatabase<ResearchProjectDef>.GetNamedSilentFail("Electricity"));

	[ItemCanBeNull]
	public static Lazy<JoyKindDef> TelevisionJoyKind =
		new(() => DefDatabase<JoyKindDef>.GetNamedSilentFail("Television"));
	...
	{
		if (PlayerCouldUseTelevision)
			return true;
		if (Find.TickManager.TicksGame < LastTickCheckedForTelevision + GenTicks.TickLongInterval)
			return false;

		PlayerCouldUseTelevision = Find.Maps.Exists(m =>
			m.IsPlayerHome
			&& m.listerBuildings.ColonistsHaveBuilding(t =>
	...
	protected override ThoughtState ShouldHaveThought(Pawn p)
	{
		if (
			...
			|| !(
				p.Faction.def.techLevel >= TechLevel.Industrial
				|| (ElectricityResearch.Value?.IsFinished ?? false)
				|| PlayerHasTelevision()
			)
			|| p.IsSlave
			|| p.needs?.mood?.thoughts?.memories == null
			|| !(p.Ideo?.HasPrecept(MemeSuperPackDefOf.MSSMeme_RimRim_Demanded) ?? false)
			|| p.needs.mood.thoughts.memories.Memories.Exists(m =>
				WatchedRimRimThoughts.Value.Contains(m.def)
			)
		)
	...
	public static bool CanHaveThought(Pawn pawn) =>
		...
		&& MemeSuperPackDefOf.MSSMeme_RimRim_Missed.Worker.CurrentState(pawn).Active;
}
