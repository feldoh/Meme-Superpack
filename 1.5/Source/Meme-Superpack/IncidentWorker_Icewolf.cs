...
public class IncidentWorker_Icewolf : IncidentWorker
{
	...
	protected override bool CanFireNowSub(IncidentParms parms) =>
		MemeSuperpackMod.settings.icewolf
		&& base.CanFireNowSub(parms)
		&& Find.CurrentMap.mapPawns.AllPawns.Exists(p =>
	...
	protected override bool TryExecuteWorker(IncidentParms parms)
	{
		...
		Pawn iceWolf = Find.CurrentMap.mapPawns.AllPawns.Find(p =>
			p.Name?.ToStringFull?.ToLowerInvariant().Contains("icewo") ?? false
		);
		if (iceWolf == null)
			return false;
		...
		if (SendLetter)
			SendStandardLetter(
				parms,
				new LookTargets(iceWolf),
				new NamedArgument(iceWolf.Name.ToStringFull, "ICEWOLF_name")
			);
		return true;
	}
}
