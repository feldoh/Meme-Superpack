...
namespace MSS.MemeSuperpack
{
	public class CompAbilityEffect_ComeAnimal : CompAbilityEffect_WithDuration
	{
		...
		public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
		{
			...
			if (pawnKind == null || !TryFindEntryCell(map, out IntVec3 entryCell))
				return;
			...
			Log.Message(
				$"{numberToWanderIn} {labelPlural} will hang around for {lurkTime} ticks at {target}"
			);
			for (var index = 0; index < numberToWanderIn; ++index)
			{
				...
				if (target.IsValid)
					newPawn.mindState.forcedGotoPosition = CellFinder.RandomClosewalkCellNear(
						target.Cell,
						map,
						10
					);
				...
				{
					newPawn.mindState.mentalStateHandler.TryStartMentalState(
						MentalStateDefOf.ManhunterPermanent
					);
					return;
				}

				if (Props.fightTarget && target.HasThing)
					newPawn.mindState.enemyTarget = target.Thing;
				if (
					!Props.fightWithSummoner
					|| !newPawn.mindState.mentalStateHandler.TryStartMentalState(
						MentalStateDefOf.BerserkWarcall,
						causedByPsycast: true
					)
				)
					return;
				newPawn.mindState.mentalStateHandler.CurState.sourceFaction = parent.pawn.Faction;
				newPawn.mindState.mentalStateHandler.CurState.forceRecoverAfterTicks = GetDurationSeconds(
						parent.pawn
					)
					.SecondsToTicks();
			}
