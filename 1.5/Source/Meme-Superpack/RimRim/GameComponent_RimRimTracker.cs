...
public class GameComponent_RimRimTracker : GameComponent
{
	...
	public override void GameComponentTick()
	{
		...
		var pawns = Find.CurrentMap.mapPawns.FreeColonistsSpawned.Where(p => p.ideo?.Ideo?.HasPrecept(MemeSuperPackDefOf.MSSMeme_RimRim_Demanded) ?? false).ToList();
		if (MemeSuperpackMod.settings.autoRimRim && Find.FactionManager.OfPlayer.ideos.GetPrecept(MemeSuperPackDefOf.MSSMeme_RimRim_Ritual) is Precept_Ritual ritual)
		{
			AutoStartRitual(Find.CurrentMap, ritual, pawns);
		}

		if (!MemeSuperpackMod.settings.whereRimRim || !pawns.Any(p => MemeSuperPackDefOf.MSSMeme_RimRim_Missed.Worker?.CurrentState(p).Active ?? false)) return;
	...
	{
		if (_cachedTelly is { Spawned: true } && (_cachedTelly.TryGetComp<CompPowerTrader>()?.PowerOn ?? false)) return _cachedTelly;
		_cachedTelly = map.listerBuildings.allBuildingsColonist
			.Find(b => b.Spawned &&
			           (b.def?.building?.joyKind ?? JoyKindDefOf.Social) == ThoughtWorker_RimRimDemanded.TelevisionJoyKind.Value &&
	...
	public void AutoStartRitual(Map map, Precept_Ritual ritual, List<Pawn> pawns)
	{
		...
		if (television == null || freeTimePawns.Count == 0) return;
		RitualRoleAssignments assignments = new RitualRoleAssignments(ritual, new TargetInfo(television));
		...
		ritual.behavior.TryExecuteOn(television, freeTimePawns.First(), ritual, null, assignments, true);
	}

	public List<Pawn> GetPawnsInFreeTime(IEnumerable<Pawn> pawns) =>
		pawns.Where(pawn => pawn.timetable.CurrentAssignment == TimeAssignmentDefOf.Anything ||
		                    pawn.timetable.CurrentAssignment == TimeAssignmentDefOf.Joy &&
		                    !pawn.DeadOrDowned &&
		                    !pawn.Drafted &&
		                    pawn.IsColonistPlayerControlled).ToList();
