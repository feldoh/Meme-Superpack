...
public class DarkSolarPowerOutput
{
	...
	public static IEnumerable<MethodBase> TargetMethods()
	{
		...
		if (ModLister.GetActiveModWithIdentifier("vanillaexpanded.vfepower") != null)
			yield return AccessTools.PropertyGetter(
				AccessTools.TypeByName("VanillaPowerExpanded.CompPowerAdvancedSolar"),
				"DesiredPowerOutput"
			);
	}

	private static readonly Lazy<float> FullSunPowerVfePower =
		new(
			() =>
				(float)
					AccessTools
	...
	private static readonly PropertyInfo RoofedPowerOutputFactorBaseGame = AccessTools.Property(
		typeof(CompPowerPlantSolar),
		"RoofedPowerOutputFactor"
	);

	private static readonly Lazy<PropertyInfo> RoofedPowerOutputFactorVfePower =
		new(
			() =>
	...
	{
		if (
			GameComponent_MemeTracker.Instance?.CurrentGaslightingTopic
			!= GameComponent_MemeTracker.GaslightingTopic.DarkSolar
		)
			return true;
		__result =
			Mathf.Lerp(
				__instance is CompPowerPlantSolar
					? -__instance.Props.PowerConsumption
					: FullSunPowerVfePower.Value,
				0,
			...
			* (float)
				(
					__instance is CompPowerPlantSolar
						...
						: RoofedPowerOutputFactorVfePower.Value
				).GetValue(__instance);
		return false;
	}
}
